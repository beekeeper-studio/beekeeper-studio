...
const logger = createLogger('db:clients:cockroachdb');
...
export default async function (server, database) {
  ...
  logger().debug('create driver client for cockroachdb with config %j', dbConfig);
...
export async function listTableTriggers(conn, table, schema) {
  // CockroachDB currently does not support triggers.
  return []
}
  ...
  return {
    execute() {
      return runWithConnection(conn, async (connection) => {
        ...
        const dataSessionId = await driverExecuteQuery(connClient, {
          query: `SHOW session_id`,
        });

        sessionId = dataSessionId.rows[0].session_id;
    ...
    async cancel() {
      ...
      canceling = true;
      try {
        const dataQueryId = await driverExecuteQuery(connClient, {
          query: `SELECT query_id FROM [SHOW QUERIES] WHERE session_id = ${sessionId} AND query = ${queryText}`,
        });

        queryId = dataSessionId.rows[0].query_id;

        if (!data.rows[0].pg_cancel_backend) {
          throw new Error(`Failed canceling query with sessionId ${sessionId}.`);
        }

        const data = await driverExecuteQuery(conn, {
          query: `CANCEL QUERY ${queryId};`,
        });
