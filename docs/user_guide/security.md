...
Beekeeper Studio already ignores `~/.ssh/config` entirely unless it is owned by you and is not writable by group or other users. To additionally skip `Match exec` sections, set `allowSshConfigMatchExec = false` in the `[security]` section of your config file. `Host` blocks and non-exec `Match` rules (`Match host`, `Match user`, etc.) are still applied — only `Match exec` is skipped.
