...
// TODO: Stage 1 ships read-only support. Features the driver can support,
// pending implementation:
//   - Table data editing (executeApplyChanges): HanaConn already exposes
//     setAutoCommit/commit/rollback; UPDATE/INSERT/DELETE round-trips work.
//   - Transactions / manual commit: flip `transactions` in supportedFeatures
//     and remove `manualCommit`/`transactions` from disabledFeatures once
//     data editing lands.
//   - Import from file: Statement.execBatch gives efficient bulk INSERT.
//   - Bind parameters: conn.exec/prepare accept `?` placeholders.
//   - Schema DDL (create/alter/drop table, indexes): HanaChangeBuilder has the
//     ALTER grammar stubbed; needs existing-column-type tracking and UI wiring.
//   - Comments on tables/columns: COMMENT ON is generated by the builder.
//   - Generated columns: GENERATION_TYPE is already read from the catalog.
//   - Custom routines: CALL + Statement.getParameterInfo() for in/out params.
//   - Partition listing: SYS.M_TABLE_PARTITIONS.
//   - CREATE SCHEMA as "add database".
//   - Kerberos authentication (driver supports it; needs UI + testing infra).
//   - LOB streaming: sendParameterData/ResultSet.getData for large values.
// Not supportable through the SQL driver: server-side backup/restore,
// charset/collation listing (HANA is Unicode-only), materialized views
// (HANA uses calculation views instead), and a database shell.
...
export class HanaClient extends BasicDatabaseClient<HanaResult> {
  ...
  private buildFilterString(filters: TableFilter[]) {
    ...
    if (filters && filters.length > 0) {
      ...
      // joinFilters interpolates each filter's `op` verbatim; restrict it to
      // AND/OR before it reaches the SQL string.
      const safeOps = filters.map((item) => ({
        ...item,
        op: (`${item.op}`.toUpperCase() === 'OR' ? 'OR' : 'AND') as 'AND' | 'OR',
      }));
      filterString = "WHERE " + joinFilters(allFilters, safeOps)
    }
    return filterString
  }
}
