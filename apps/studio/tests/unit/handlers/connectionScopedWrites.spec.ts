...
// A @Min(1) validator on each model's connectionId rejects the write: -1 is a
// shared bucket every unsaved session would pile into. The OpenTab history
// queries bail out early for the same reason, since an absent id can widen a
// scoped delete into "every connection's rows".
...
describe('connection-scoped writes', () => {
  ...
  it.each(NO_CONNECTION)('rejects a tab keyed on %p', async (connectionId) => {
    await expect(AppDbHandlers['appdb/tabs/save']({
      obj: {
        tabType: 'query', title: 'scratch', position: 1,
        connectionId, workspaceId: WORKSPACE_ID,
      },
      options: {},
    } as any)).rejects.toThrow(/connectionId/)

    expect(await OpenTab.count({ withDeleted: true })).toBe(0)
  })

  it.each(NO_CONNECTION)('rejects query history keyed on %p', async (connectionId) => {
    await expect(AppDbHandlers['appdb/usedQuery/save']({
