// Typing COMMIT (or ROLLBACK) in a query tab that has an active manual-commit
// transaction is handled by submitQuery's identification branch, which calls
// toggleCommitMode() BEFORE the statement executes. toggleCommitMode's auto
// branch rolls the driver transaction back and releases the tab's reserved
// connection — so a typed COMMIT actually discards the user's work, and the
// literal COMMIT text then runs on an unreserved pool connection (a warning
// on Postgres, error 3902 on SQL Server).
//
// These tests drive the real submitQuery/toggleCommitMode methods with a spy
// connection and assert the intended sequencing: the typed statement must run
// while the tab still holds its reserved connection, and nothing may issue a
// driver-level rollback for a typed COMMIT. They fail until the ordering is
// fixed.
//
// Mounting TabQueryEditor would drag in the whole editor surface, so the
// methods are exercised directly (same approach as CoreTabsClose.spec.ts).
// The editor/CodeMirror imports are mocked — none of them participate in the
// logic under test.
