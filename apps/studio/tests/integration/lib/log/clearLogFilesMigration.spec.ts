...
describe('20260527_clear_log_files migration', () => {
  ...
  it('removes rotated *.old files', async () => {
    write('main.old')
    write('utility.old')
    write('renderer.old')
    write('keep-me.log')
    ...
    expect(exists('main.old')).toBe(false)
    expect(exists('utility.old')).toBe(false)
    expect(exists('renderer.old')).toBe(false)
  ...
  it('keeps going if one *.old deletion fails', async () => {
    write('one.old', 'a')
    write('two.old', 'b')

    const original = fs.unlinkSync
    const spy = jest.spyOn(fs, 'unlinkSync').mockImplementation((file) => {
      if (String(file).endsWith('one.old')) {
        throw new Error('locked')
      }
    ...
    expect(exists('two.old')).toBe(false)
    spy.mockRestore()
  })
