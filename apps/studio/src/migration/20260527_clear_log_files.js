...
// redacter was wired up. No schema change — this migration just truncates
// existing log files and removes rotated *.old artifacts. Each operation
// is independently wrapped so one failure (missing file, permission denied,
// locked file) doesn't skip the others.
...
function unlinkOldFile(filePath) {
  try {
  ...
  async run() {
    ...
    for (const entry of entries) {
      if (entry.endsWith('.old')) {
        unlinkOldFile(path.join(logsDir, entry))
      }
    }
  },
}
