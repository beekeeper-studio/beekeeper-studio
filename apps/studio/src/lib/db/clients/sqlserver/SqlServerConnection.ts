...
export class SqlServerConnection extends DatabaseConnection<Request | Transaction> {
  ...
  protected async doConnect(options: ConnectOptions): Promise<void> {
    const dbConfig = await this.configDatabase(this.server, this.database, options.signal)
  ...
  protected doGetClient(): Promise<Request | Transaction> {
    throw new Error("Please use `.request() or `.transaction()` instead.");
  }
