...
export abstract class BasicDatabaseClient<RawResultType> implements DatabaseClient {
  ...
  abstract getTableReferences(table: string, schema?: string): void;
  ...
  abstract executeQuery(queryText: string): Promise<NgQueryResult[]>;
  ...
  abstract selectTop(table: string, offset: number, limit: number, orderBy: OrderBy[], filters: string | TableFilter[], schema?: string): Promise<TableResult>;
  ...
  protected abstract rawExecuteQuery(q: string, options: any): Promise<RawResultType>

  async driverExecuteQuery(q: string, options: any): Promise<RawResultType> {
