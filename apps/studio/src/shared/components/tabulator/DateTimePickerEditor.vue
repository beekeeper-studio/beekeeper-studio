  ...
  watch: {
    ...
    typeEditorActive() {
      if (this.typeEditorActive) {
        ...
        let dataValue =
          refValue == null ? refValue : helpers.niceString(refValue);

        if (this.isTimeType(dataType) && dataValue !== null) {
          dataValue =
            dataValue.search(/(\+|-)/i) > -1 && !isNaN(dataValue.slice(-1))
              ? `${dataValue}:00`
              : dataValue;
      ...
      this.$set(
        this.$refs.nullable.$data,
        "typeEditorActive",
        this.typeEditorActive
      );
    },
  },
  mounted() {
