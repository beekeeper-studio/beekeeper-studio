  ...
  <div class="nav-item-wrap" v-hotkey="keymap">
    ...
    >
      <a
        ...
        @contextmenu="
          $bks.openMenu({ item: tab, options: contextOptions, event: $event })
        "
      ...
      >
        <div class="dialog-content">
          <div class="dialog-c-title">
            Really close
            <span class="tab-like"
              ><tab-icon :tab="tab" :force-icon="true" />
              {{ this.tab.title }}</span
            >?
          </div>
          <p>
            {{
              tab.isRunning
                ? "There is an active process still running. Closing the tab now will force it to stop and could result in catastrophe."
