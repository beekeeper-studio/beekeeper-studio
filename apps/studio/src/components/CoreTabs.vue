...
export default Vue.extend({
  ...
  methods: {
    ...
    async importSqlFiles(paths: string[]) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          const text = this.$util.send('file/read', { path: file.path, options: { encoding: 'utf8', flag: 'r' }})
          if (text) {
    ...
    async handlePromptQueryExport(query) {
      ...
      try {
        this.$util.send('file/write', { path: filePath, text: query.text, options: { encoding: 'utf8' }})
