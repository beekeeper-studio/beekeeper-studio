  ...
  <div class="core-tabs" v-hotkey="keymap">
    <div class="tabs-header">
      ...
      <a
        ...
        v-tooltip="
          'Upgrade for: backup/restore, import from file, larger query results, and more!'
        "
        ...
        <div v-kbd-trap="true">
          <div class="dialog-content">
            <div class="dialog-c-title">
              Really {{ this.dbAction | titleCase }}
              <span class="tab-like"
                ><tab-icon :tab="tabIcon" /> {{ this.dbElement }}</span
              >?
            </div>
        ...
        <div v-kbd-trap="true">
          ...
          >
            <div class="dialog-c-title text-center">
              Table Duplication not supported for {{ this.dialectTitle }} yet.
              Stay tuned!
            </div>
          </div>
          <div class="dialog-content" v-else>
            <div class="dialog-c-title">
              {{ this.dbAction | titleCase }}
              <span class="tab-like"
                ><tab-icon :tab="tabIcon" /> {{ this.dbElement }}</span
              >?
            </div>
            ...
            <small
              >This will create a new table and copy all existing data into it.
              Keep in mind that any indexes, relations, or triggers associated
              with the original table will not be duplicated in the new
              table</small
            >
          </div>
          <div
    ...
    <confirmation-modal :id="confirmModalId">
      ...
      <template v-slot:message> You will lose unsaved changes </template>
    </confirmation-modal>
  ...
  computed: {
    ...
    rootBindings() {
      return [
        ...
        {
          event: AppEvent.deleteDatabaseElement,
          handler: this.deleteDatabaseElement,
        },
        {
          event: AppEvent.dropDatabaseElement,
          handler: this.dropDatabaseElement,
        },
        {
          event: AppEvent.duplicateDatabaseTable,
          handler: this.duplicateDatabaseTable,
        },
        { event: AppEvent.dropzoneDrop, handler: this.handleDropzoneDrop },
        {
          event: AppEvent.promptQueryExport,
          handler: this.handlePromptQueryExport,
        },
  ...
  methods: {
      ...
      this.$nextTick(async () => {
        try {
          if (this.dbAction.toLowerCase() === "drop") {
            await this.connection.dropElement(
              dbName,
          ...
          if (this.dbAction.toLowerCase() === "truncate") {
            await this.connection.truncateElement(
              dbName,
              entityType?.toUpperCase(),
              schema
            );
    ...
    async duplicateTableSql() {
      ...
      if (
        ...
        this.duplicateTableName === "" ||
        this.duplicateTableName === undefined
      ) {
      ...
      try {
        const sql = await this.connection.duplicateTableSql(
          tableName,
          this.duplicateTableName,
          schema
        );
        ...
        this.$noty.error(
          `Error printing ${this.dbAction} query: ${ex.message}`
        );
      } finally {
    ...
    async duplicateTable() {
      ...
      this.$nextTick(async () => {
        try {
          ...
          await this.connection.duplicateTable(
            tableName,
            this.duplicateTableName,
            schema
          );
    ...
    async addTab(item: TransportOpenTab) {
      const savedItem = await this.$store.dispatch("tabs/add", {
        item,
        endOfPosition: true,
      });
    ...
    async loadTableCreate(table) {
      ...
      else if (table.entityType === "materialized-view")
        method = "getMaterializedViewCreateScript";
      if (!method) {
        this.$noty.error(
      ...
      try {
        ...
        const stringResult = safeFormat(
          _.isArray(result) ? result[0] : result,
          { language: FormatterDialect(this.dialect) }
        );
        this.createQuery(stringResult);
      } catch (ex) {
        this.$noty.error(
          `An error occured while loading the SQL for '${table.name}' - ${ex.message}`
        );
        throw ex;
      }
    },
    ...
    async handleDropzoneDrop(event: DropzoneDropEvent) {
      ...
      const noty = this.$noty.info(notyText, {
        queue: notyQueue,
        allowRawHtml: true,
        buttons: [Noty.button("Abort", "btn btn-danger", abort)],
      });
      ...
      if (aborted) {
        ...
        this.$noty.error("Some files could not be loaded", {
          killer: notyQueue,
        });
      } else {
    ...
    async importSqlFiles(paths: string[]) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          const text = await this.$util.send("file/read", {
            path: file.path,
    ...
    async handlePromptQueryExport(query) {
      ...
      try {
        await this.$util.send("file/write", {
          path: filePath,
          text: query.text,
        ...
        console.error(e);
        this.$noty.error(
          "Query could not be exported. See console for details.",
          { killer: notyQueue }
        );
      }
    },
    async loadRoutineCreate(routine) {
      ...
      const stringResult = safeFormat(_.isArray(result) ? result[0] : result, {
        language: FormatterDialect(this.dialect),
      });
      this.createQuery(stringResult);
    },
    ...
    async closeAll() {
      ...
      if (unsavedTabs.length > 0) {
        const confirmed = await this.$confirm(
          "Close all tabs?",
          `You have ${unsavedTabs.length} unsaved ${window.main.pluralize(
            "tab",
            unsavedTabs.length
          )}. Are you sure?`
        );
