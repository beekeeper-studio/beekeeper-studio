...
async function applyThemeCSS(
  ...
  try {
    ...
    // Instead of executing JavaScript directly, send theme data to the frontend
    const themeData = {
      name: themeName,
      // Include any fallback values if needed
      fallbacks: {
        bgColor: themeName === 'light' ? '#ffffff' : '#1e1e1e',
        textColor: themeName === 'light' ? '#333333' : '#d4d4d4'
      }
    };

    // Send theme data to frontend via IPC
    webContents.send('theme-changed', themeData);

    // Log theme application
    console.log(`Theme data sent to frontend: ${themeName}`);
