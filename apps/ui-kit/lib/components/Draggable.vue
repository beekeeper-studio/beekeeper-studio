...
export default Vue.extend({
  ...
  props: {
    ...
    options: {
      type: Object as PropType<Partial<SortableOptions>>,
      default: () => ({}),
    },
  ...
  methods: {
    sourceInstance(e: SortableEvent): DraggableInstance | null {
      ...
      return from
        ? ((from as any).__draggable__ as DraggableInstance | undefined) ??
            null
        : null;
    },
    ...
    handleAdd(e: SortableEvent) {
      ...
      const element =
        sourceList != null && e.oldIndex != null
          ? sourceList[e.oldIndex]
          : null;
      const list = this.realList;
      ...
      this.$emit("change", {
        added: { element, newIndex: e.newIndex },
      });
    },
    handleRemove(e: SortableEvent) {
      ...
      if (list == null || e.oldIndex == null) {
        this.$emit("change", {
          removed: { element: null, oldIndex: e.oldIndex },
        });
        return;
      }
      const updated = list.slice();
