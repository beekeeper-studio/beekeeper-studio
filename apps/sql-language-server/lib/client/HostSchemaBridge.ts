 ...
 *
 * NOTE: this object owns its own JSON-RPC connection on the worker — it
 * does not share the LSP client's connection. JSON-RPC routes by method
 * name, so the bridge and the host's LSP client can coexist on the same
 * Worker as long as their method namespaces don't collide (LSP methods
 * are unprefixed; ours are `bks/...`).
 */
export class HostSchemaBridge {
  private connection: MessageConnection;
  private disposers: { dispose(): void }[] = [];

  constructor(worker: Worker, private provider: SchemaProvider) {
