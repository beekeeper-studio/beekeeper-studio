...
-- and optionally clicks Allow / Don't Allow on it.
-- Usage: osascript alerts.applescript list
--        osascript alerts.applescript allow|deny
on run argv
				...
				try
					...
					repeat with w in (every window of p)
						...
						if (count of btns) > 0 then
							...
							if mode is not "list" and t contains "local network" then
	...
	return "NO LOCAL NETWORK ALERT FOUND. Windows with buttons:" & linefeed & out
end run
